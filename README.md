...
**The full results live in the interactive explorer at <https://sql-ast-benchmark.luca.phd>**, with per-dialect distribution charts, correctness tables, per-file coverage, and per-parser repository metadata, all kept current with the latest benchmark run. This README is a stable overview; see [CHANGELOG.md](CHANGELOG.md) for project history.
